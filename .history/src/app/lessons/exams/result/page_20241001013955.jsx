...
const ResultPage = () => {
  ...
  const [myNumber, setMyNumber] = useState(0); // State for number of stars
  const [seconds, setSeconds] = useState("0"); // Default value for seconds
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const myNumberValue = myData ? parseFloat(myData) : 0;
      setMyNumber(myNumberValue); // Set the state

      const calculatedResult = (myNumberValue / 3) * 100;
  ...
  const handleButtonClick = () => {
    if (myNumber !== 3) {
      router.back(); // Go back if not passed
    } else {
      // Implement the next stage logic here
      router.push('/next-stage'); // Replace with your actual next stage path
    }
  };
    ...
    <motion.div variants={pageVariants} initial="hidden" animate="visible">
      <section className={poppins.className}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "70px" }}>
          {
            myNumber === 3 ?
              <>
                <Star style={{ fontSize: 100, color: '#FFD700', marginRight: "70px" }} />
                <Star style={{ fontSize: 100, color: '#FFD700', marginRight: "70px" }} />
                <Star style={{ fontSize: 100, color: '#FFD700' }} />
              </> :
              myNumber === 2 ? 
                <>
                  <Star style={{ fontSize: 100, color: '#FFD700', marginRight: "70px" }} />
                  <Star style={{ fontSize: 100, color: '#FFD700', marginRight: "70px" }} />
                  <Star style={{ fontSize: 100, color: 'gray' }} />
                </> :
                myNumber === 1 ?
                  <>
                    <Star style={{ fontSize: 100, color: '#FFD700', marginRight: "70px" }} />
                    <Star style={{ fontSize: 100, color: 'gray', marginRight: "70px" }} />
                    <Star style={{ fontSize: 100, color: 'gray' }} />
                  </> :
                  <>
                    <Star style={{ fontSize: 100, color: 'gray', marginRight: "70px" }} />
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: "90px" }}>
          {
            myNumber !== 3 ?
              <p style={{ fontSize: "40px" }}>You must achieve 100% to move to the next stage</p> :
              <p style={{ fontSize: "40px" }}>Congratulations You Passed the Exam</p>
          }
        </div>
        <div style={{ display: "flex", justifyContent: "center", marginTop: "40px" }}>
          <button onClick={handleButtonClick} style={{ borderRadius: "6px", width: "161px", height: "56px" }}>
